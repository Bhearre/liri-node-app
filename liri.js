...
async function searchSpotify(songName) {
    ...
    try {
        const data = await spotify.searchTracks(songName);
        ...
        for (let i = 0; i < items.length; i++) {
            const item = items[i] // Current Item we're working with
...
function searchBandsInTown(bandName) {
    const url = "https://rest.bandsintown.com/artists/" + bandName + "/events?app_id=codingbootcamp";
    axios.get(url)
        .then(function (response) {
            ...
            for (let i = 0; i < response.data.length; i++) {
                ...
                const startTime = format(new Date(event.datetime), 'MM/dd/yyyy');
                table.push(
                    [
                        event.venue.name,
                        location,
...
// ===============================  Code Below Will Use the "Search" Endpoint of the OMDB API and return 10 results. ===========
    // const url = "http://www.omdbapi.com/?s=" + movieName + "&apikey=e4179ba0&page=1&type=movie&Content-Type=application/json";
    // axios.get(url)
    //     .then(function (response) {
    //         const searchMe = response.data.Search;
    ...
    //     for (let k = 0; k < searchMe.length; k++) {

    //  const searchTitle = searchMe[k].Title;
    //const searchTitle = searchTable[k].Title;
    //console.log("this is searchTitle " + searchTitle);
// ========================================End code to Search for Multiple titles ======================================

    const url = "http://www.omdbapi.com/?t=" + movieName + "&apikey=e4179ba0";
    axios.get(url)
        .then(function (response) {
            const thisTitle = response.data;
...
function doRandom() {
    ...
    fs.readFile('./random.txt', 'utf8', function (err, contents) {
        if (err) {
            console.error('Error reading random.txt:', err);
            return;
        }
